...
CREATE OR REPLACE TABLE ${PROJECT_NAME}.${DATASET_NAME}._tmp_Products
  OPTIONS (
    expiration_timestamp = TIMESTAMP_ADD(CURRENT_TIMESTAMP(), INTERVAL 120 MINUTE))
AS
WITH
  Lia AS (
    SELECT DISTINCT
...
WITH
  Benchmarks AS (
    ...
    FROM ${PROJECT_NAME}.${DATASET_NAME}.MEX_benchmark_region AS BR
    INNER JOIN
      ${PROJECT_NAME}.${DATASET_NAME}.MEX_benchmark_details AS BD
      ON BD.metric_name = BR.metric_name
    -- The region filter here should be on the setup process
    WHERE BR.region_name = ${REGION}
  ),
  TotalProducts AS (
    SELECT
      merchant_id,
...
SELECT
  ...
  metric_value,
  total_products
FROM BenchmarksPerMerchant
INNER JOIN TotalProducts
  USING (
    merchant_id,
    channel,
    targeted_country,
