...
from acit import product as product_category


class PerformanceMaxTest(parameterized.TestCase):
  ...
  def test_dimension_is_wildcard(self, dimension, expected):
    matcher = performance_max.TargetingMatcher(
        category_cache=product_category.ProductCategoryCache()
    )

    self.assertEqual(
        matcher.dimension_is_wildcard(dimension),
        expected,
    )
  ...
  def test_specific_dimension_matches_product(
    ...
    self.assertEqual(
        matcher.dimension_matches_product(product, dimension),
        expected,
    )
  ...
  def test_product_targeted_by_tree(self, product, tree, expected):
    ...
    self.assertEqual(matcher.product_targeted_by_tree(product, tree), expected)
