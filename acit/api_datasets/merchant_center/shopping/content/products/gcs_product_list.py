  ...
  with open(_PRODUCTS_FILE, 'wt') as f:
    while account_request is not None:
      ...
      for account in accounts:
        ...
        print(account_id)
        product_request = service.products().list(merchantId=account_id)
        while product_request is not None:
          result2 = product_request.execute()
          products = result2.get('resources')
          if not products:
            print(f'No products were found for account {account_id}.')
            break
          for product in products:
            product['merchantId'] = account_id
            product['aggregatorId'] = merchant_id
